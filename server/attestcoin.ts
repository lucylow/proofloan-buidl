...
export function previewAttestcoinFacts(walletAddress: string, sourceChain: SourceChain): VerifiedFact[] {
  ...
  return [
    { id: `vf_${hashValue({ root, n: 1 })}`, chain: sourceChain, sourceBlock: sourceChain === "Ethereum Sepolia" ? 6421883 : 12804112, txHash: `${chainPrefix}a91f...c42e`, eventType: "REPAYMENT", amount: "1,250 USDC", asset: "USDC", verificationBlock: 3204118, verifiedAt: now, observedAt: new Date(Date.now() - 3 * 86_400_000).toISOString(), freshness: "Fresh", proofRoot: `${root}_a`, proofWorker: "Attestcoin proof worker" },
    { id: `vf_${hashValue({ root, n: 2 })}`, chain: sourceChain, sourceBlock: sourceChain === "Ethereum Sepolia" ? 6104220 : 12330441, txHash: `${chainPrefix}4b07...8aa1`, eventType: "COLLATERAL_DEPOSIT", amount: "2,800 USDC", asset: "USDC", verificationBlock: 3204123, verifiedAt: now, observedAt: new Date(Date.now() - 3 * 86_400_000).toISOString(), freshness: "Fresh", proofRoot: `${root}_b`, proofWorker: "Attestcoin proof worker" },
    { id: `vf_${hashValue({ root, n: 3 })}`, chain: sourceChain, sourceBlock: sourceChain === "Ethereum Sepolia" ? 5998141 : 11990008, txHash: `${chainPrefix}11f8...d912`, eventType: "REPAYMENT", amount: "850 USDC", asset: "USDC", verificationBlock: 3204127, verifiedAt: now, observedAt: new Date(Date.now() - 90 * 86_400_000).toISOString(), freshness: "Aging", proofRoot: `${root}_c`, proofWorker: "Attestcoin proof worker" },
  ];
}
